...
import { UploadCloud, Image, Play, ArrowLeft, X } from "lucide-react";
...
const UploadAvatar = () => {
  ...
  const [preview, setPreview] = useState(
    signupData.avatarFile ? URL.createObjectURL(signupData.avatarFile) : null
  );
  ...
  const handleNext = () => {
    if (!signupData.avatarFile) { setError("Please upload a profile picture."); return; }
    navigate("/upload-cover");
  };
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
          ...
          <div
            ...
            {preview ? (
              <>
                ...
                <button onClick={(e) => { e.stopPropagation(); setPreview(null); updateSignup({ avatarFile: null }); }}
          ...
          <div className="flex gap-3 mt-6">
            ...
            <button onClick={handleNext}
              className="flex-1 py-2.5 bg-gradient-to-r from-indigo-600 to-indigo-700 hover:from-indigo-700 hover:to-indigo-800 active:scale-[0.99] text-white rounded-xl text-sm font-semibold shadow-sm hover:shadow-md transition-all">
              Continue →
            </button>
          </div>
        </div>
