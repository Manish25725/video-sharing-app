    ...
    <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
      <div className="space-y-1">
        <div className="flex items-center gap-3">
          ...
          >
            <TrendingUp className="w-5 h-5" style={{ color: "#ec5b13" }} />
          </div>
          <h2 className="text-4xl font-black tracking-tight text-white">Trending</h2>
        </div>
        <p className="text-slate-400 pl-[52px]">Most popular videos right now</p>
      </div>
      <button
        onClick={handleRefresh}
        disabled={refreshing}
        ...
        onMouseEnter={e => { if (!refreshing) e.currentTarget.style.background = "rgba(236,91,19,0.22)"; }}
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {videos.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {videos.map((video, index) => (
              <div key={video.id} className="relative">
                {/* Rank badge */}
                <div
                  className="absolute top-3 left-3 z-10 flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-black"
                  style={
                    index === 0
                      ? { background: "linear-gradient(135deg,#f59e0b,#f97316)", color: "#fff", boxShadow: "0 4px 12px rgba(245,158,11,0.4)" }
                      : index === 1
                      ? { background: "linear-gradient(135deg,#94a3b8,#cbd5e1)", color: "#0f172a", boxShadow: "0 4px 12px rgba(148,163,184,0.3)" }
                      : index === 2
                      ? { background: "linear-gradient(135deg,#cd7f32,#b45309)", color: "#fff", boxShadow: "0 4px 12px rgba(180,83,9,0.35)" }
                      : { background: "rgba(0,0,0,0.7)", color: "#94a3b8", backdropFilter: "blur(4px)" }
                  }
                >
          ...
          <div className="flex flex-col items-center justify-center py-24">
            <div className="w-20 h-20 rounded-2xl flex items-center justify-center mb-6"
              style={{ background: "rgba(236,91,19,0.1)" }}>
              <TrendingUp className="w-10 h-10" style={{ color: "#ec5b13" }} />
            </div>
            <h3 className="text-xl font-bold text-white mb-2">No Trending Videos</h3>
            <p className="text-slate-400 mb-8 text-center max-w-md">
